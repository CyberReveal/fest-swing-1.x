...
public class JTreePathFinder {
    // TODO TEST
    public static final String SEPARATOR = "/u0x1f";
    ...
    @RunsInCurrentThread
    @Nonnull
    TreePath findMatchingPath(@Nonnull final JTree tree, @Nonnull final String path) {
    ...
    private @Nonnull
    LocationUnavailableException pathNotFound(@Nonnull final String path) {
        throw new LocationUnavailableException(String.format("Unable to find path %s", quote(path)));
    }

    private @Nonnull
    String[] splitPath(@Nonnull final String path) {
    ...
    private @Nonnull
    LocationUnavailableException multipleMatchingNodes(@Nonnull final String matchingText,
            @Nullable final Object parentText) {
        String msg = String
                .format("There is more than one node with value '%s' under", matchingText, quote(parentText));
        throw new LocationUnavailableException(msg);
    }

    private @Nullable
    String value(@Nonnull final JTree tree, @Nullable final Object modelValue) {
    ...
    void separator(@Nonnull final String newSeparator) {
        this.separator = newSeparator;
    }

    void cellReader(@Nonnull final JTreeCellReader newCellReader) {
