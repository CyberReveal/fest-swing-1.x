...
public class JTableHeaderLocation {
     ...
     */
    @RunsInCurrentThread
    public @Nonnull Pair<Integer, Point> pointAt(@Nonnull final JTableHeader tableHeader,
            @Nonnull final TextMatcher matcher) {
    ...
    @RunsInCurrentThread
    private boolean isValidIndex(@Nonnull final JTableHeader tableHeader, final int index) {
     ...
     */
    @RunsInCurrentThread
    public @Nonnull Point pointAt(@Nonnull final JTableHeader tableHeader, final int index) {
        return point(tableHeader, checkIndexInBounds(tableHeader, index));
    }

    @RunsInCurrentThread
    private static @Nonnull Point point(@Nonnull final JTableHeader tableHeader, final int index) {
        Rectangle r = tableHeader.getHeaderRect(index);
        return new Point(r.x + r.width / 2, r.y + r.height / 2);
    }

    @RunsInCurrentThread
    private int checkIndexInBounds(@Nonnull final JTableHeader tableHeader, final int index) {
        int itemCount = columnCount(tableHeader);
    ...
    /**
     * <p>
     * Returns the index of the column which name matches the value in the given {@link TextMatcher}, or -1 if a
     * matching column was not found.
     * </p>
     * <p>
     * <b>Note:</b> This method is accessed in the current executing thread. Such thread may or may not be the event
     * dispatch thread (EDT.) Client code must call this method from the EDT.
     ...
     */
    @RunsInCurrentThread
    public int indexOf(@Nonnull final JTableHeader tableHeader, @Nonnull final TextMatcher matcher) {
    ...
    @RunsInCurrentThread
    private int columnCount(@Nonnull final JTableHeader header) {
        return header.getColumnModel().getColumnCount();
    }

    @RunsInCurrentThread
    private String columnName(@Nonnull final JTableHeader tableHeader, final int index) {
